    ...
    for sql_i in SQL_name_list:
        ...
        while x == 0:
            if sql_list_i[0]+'{'+str(y)+'}' in SQL_list:
                ...
                x = 1
        SQL_list[sql_list_i[0]+'{'+str(y)+'}'] = (sql_list_i[1], sql_list_i[2], sql_list_i[3])
    ...
    for SQL_list_ob in SQL_list:
        for day_ob in day:
            if SQL_list[SQL_list_ob][0] == 'day':
                ...
                print('取数粒度设置错误，请检查！！')
                exit()
            print(''.join(('尝试打开SQL脚本:',SQL_list_ob[:SQL_list_ob.find('{')],'.sql ...')))
            try:
                ff = open(''.join((SQL_path,'\\',SQL_list_ob[:SQL_list_ob.find('{')],'.sql')), 'r')
                print(''.join((SQL_list_ob[:SQL_list_ob.find('{')],'.sql 已打开。')))
            except:
                print('SQL脚本：', SQL_list_ob[:SQL_list_ob.find('{')], ' 不存在，请检查！')
                continue
            if SQL_list[SQL_list_ob][0] == 'para':
                ...
                try:
                    sql_ob = ff.read().replace('&1', star_time).replace('&2', end_time)
                except:
                    print('SQL日期设置异常，请检查！')
                    continue
            print('查询开始,请稍后...')
            ...
            if SQL_list[SQL_list_ob][2][0] == '$':
                name_down = ''.join((SQL_list[SQL_list_ob][2][1:],'_',db_ob,'.csv'))
                f = open(''.join((SQL_list[SQL_list_ob][1], '\\', name_down)), 'a', encoding='utf-8-SIG')
                ...
                while k == 0:
                    if l == 0:
                        if ''.join((SQL_list[SQL_list_ob][2],'_',day_ob,'_',db_ob,'.csv')) in local_dir_list:
                            l += 1
                        else:
                            k=1
                    else:
                        if ''.join((SQL_list[SQL_list_ob][2],'_',day_ob,'_',db_ob,'(',str(l),').csv')) in local_dir_list:
                            l += 1
                        else:
                            k = 1
                    ...
                    if l == 0:
                        name_down = ''.join((SQL_list[SQL_list_ob][2],'_',day_ob,'_',db_ob,'.csv'))
                    else:
                        name_down = ''.join((SQL_list[SQL_list_ob][2],'_',day_ob,'_',db_ob,'(',str(l),').csv'))
                    f = open(''.join((SQL_list[SQL_list_ob][1],'\\',name_down)), 'w', encoding='utf-8-SIG')
