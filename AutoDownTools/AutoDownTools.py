...
import os,paramiko,stat,configparser,sys,datetime,subprocess,time
...
class Getini:
    ...
    def autodowntools(self):
        ...
        if len(db_name_list) == 0:
            ...
            for db_i in db_name_list:
                ...
                if db_list_i[0] in self.db_list:
                    ...
                    if len(db_list_i) == 3:
                        self.db_list[db_list_i[0]] = (db_list_i[1], db_list_i[2])
                    else:
                        self.db_list[db_list_i[0]] = (self.cf.get('key', 'user'), self.cf.get('key', 'pwd'))
        ...
        if self.timing in ['','0']:
            ...
            if int(time.strftime('%H', time.localtime(time.time()))) == 0 and self.timing_dyn == '1':
                self.local_path = '/'.join((self.local_path, (datetime.date.today() - datetime.timedelta(days=1)).strftime('%Y%m%d')))
                self.filter_2_1 = [(datetime.date.today() - datetime.timedelta(days=1)).strftime('%Y%m%d')]
            else:
                self.local_path = '/'.join((self.local_path, datetime.date.today().strftime('%Y%m%d')))
            ...
            for down_type_2 in down_list[0][down_type_1]:
                for down_file in down_list[0][down_type_1][down_type_2]:
                    ...
                    if classify == '0':
                        ...
                        self.new_local_path = '/'.join((local_path, down_type_1))
                        ini.new_local_path = '/'.join((local_path, down_type_1))

                    elif classify == '2':
                        self.new_local_path = '/'.join((local_path, ini.filter_2[down_type_2][0]))
                        ini.new_local_path = '/'.join((local_path, ini.filter_2[down_type_2][0]))
                        ...
                        sys.exit()
                    new_local_file = ''.join((prefix, prefix_ip,down_file[down_file.rfind('/')+1:]))
                    new_local_fullname = '/'.join((self.new_local_path,new_local_file))
            ...
            for i in self.filelist[0]:
                for j in filter1:
                    for k in filter2:
                        ...
                        if (j.lower() in i_filename) and (k.lower() in i_filename) and (filetype.lower() in i_filename):
                            try:
...
class Progress():
    ...
    def progress(self,count_,filename = ''):
        ...
        sys.stdout.write("\r  [ %s %s %d ]  %d%%  <%s>"%(str(count_),hashes + spaces, self.len_,int(count_/self.len_*100),filename))
        sys.stdout.flush()
    ...
    for ip in ip_list:
        ...
        if ip_db.status == 1:
            ...
            if getfile.filterlist[1] == 0:
                ...
                ip_db.down_main(ini.local_path,getfile.filterlist,ini.prefix_ip,ini.prefix,ini.classify)
            print('\n')
            print('>>> Done')
        else:
            continue
